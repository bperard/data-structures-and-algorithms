...
const screenForNames = (arr) => {
  // Solution code here...
  return arr.filter(string => /^(Mr|Mrs|Ms|Dr)\.\s{1}\w+$/.test(string));
}
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  return arr.filter(character => +character.mass > +arr[0].mass).map(character => character.name).join(' - ');
};
...
const detectTicTacToeWin = (board) => {
  ...
  const verifyOrNope = (curr, spot) => {
    const lossCheck = curr === spot && spot !== '' ? curr : curr === spot && spot === '' ? 'nope' : curr === 'done' ? 'done' : curr === 'nope' ? 'done' : 'nope';
    if (lossCheck === 'nope') {
      col.possibleWins--;
    }
    return lossCheck;
  ...
  board.forEach((row, rowIndex) => {
    ...
    // Single Row
    row.reduce((tracker, mark, index) => {

      // Initial marks
      if (rowIndex === 0) {
        col[index] = mark;
        if (col.diagFor.index === index) {
          col.diagFor.mark = mark;
        }
        if (col.diagRev.index === index) {
          col.diagRev.mark = mark;
        }
      }

      // Check and proceed
      if (col.diagFor.index === index) {
        col.diagFor.mark = verifyOrNope(col.diagFor.mark, mark);
      }
      if (col.diagRev.index === index) {
        col.diagRev.mark = verifyOrNope(col.diagRev.mark, mark);
      }

      col[index] = verifyOrNope(col[index], mark);
      return verifyOrNope(tracker, mark);
    }, null);
