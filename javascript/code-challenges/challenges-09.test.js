...
const updateNumbers = (obj) => {
  ...
  for (let name in obj) {
    nameNumberArray.push(`${name}: ${obj[name]}`);
  }
...
const hasChildrenValues = (arr, character) => {
  ...
  return arr.map(currCharacter => {
    if (Object.values(currCharacter).includes(character)) {
      return 'children' in currCharacter;
    }
...
const houseSize = (arr) => {
  ...
  for (let character of arr) {
    ...
    if ('children' in character) {
      currMembers += character.children.length;
    }
    sizes.push({
...
const houseSurvivors = (arr) => {
  ...
  for (let character of arr) {
    let currMembers = 0;
    currMembers++;
    if (character.spouse && !deceasedSpouses.includes(character.spouse)) {
      currMembers++;
    }
    if ('children' in character) {
      currMembers += character.children.length;
