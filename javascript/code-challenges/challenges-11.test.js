...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((total, currArray) => {
    const currSum = currArray.reduce((currTotal, value) => value === target ? currTotal + 1 : currTotal, 0);
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((total, currArray) => {
    const currSum = currArray.reduce((currTotal, value) => currTotal + value);
    return total + currSum;
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(array => array.filter(value => value % 5 === 0 && typeof value === 'number').map(filteredValue => Math.pow(2, filteredValue)));
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((shortest, currCharacter) => +shortest.height > +currCharacter.height ? currCharacter : shortest).name;
};
