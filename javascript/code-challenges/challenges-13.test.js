...
const longestString = (arr) => {
  // Solution code here...
  const lengthAndIndex = arr.reduce((tracker, string, index) => !tracker.length || tracker.length < string.length ? { length: string.length, index: index } : tracker, { length: null, index: -1 });
  return lengthAndIndex.index;
};
...
const findHappiness = (arr) => {
  // Solution code here...
  return arr.filter(string => /:\)/.test(string));
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  return str.split('').reduce((oddString, char, index) => index % 2 === 0 ? oddString : oddString + char, '');
};
...
const allHappy = (arr) => {
  // Solution code here...
  let notSad = true;
  for (let i = 0; i < arr.length && notSad; i++) {
    notSad = /:\)/.test(arr[i]);
  }
  return notSad;
};
...
const findEvery = (arr, target) => {
  // Solution code here...
  let yesEvery = true;
  for (let i = 0; i < arr.length && yesEvery; i++) {
    yesEvery = arr[i].includes(target);
  }
  return yesEvery;
};
...
const unenrollBrook = (arr) => {
  // Solution code here...
  return arr.map(courseRoster => courseRoster.filter(name => !name.includes('Brook')));
};
...
const sortByDay = (arr) => {
  // Solution code here...
  const weekdayIndexAndArrayObject = daysOfWeek.reduce((returnObj, day, index) => {
    returnObj[day] = index;
    returnObj.returnArray = [...returnObj.returnArray, []];
    return returnObj;
  }, {
    returnArray: []
  });
  arr.forEach(event => {
    event.split(' ').forEach(possibleDay => {
      possibleDay = possibleDay.replace(/y.$/, 'y');
      if (weekdayIndexAndArrayObject[possibleDay] > -1) {
        weekdayIndexAndArrayObject.returnArray[weekdayIndexAndArrayObject[possibleDay]].push(event);
      }
    });
  });
  return weekdayIndexAndArrayObject.returnArray;
};
